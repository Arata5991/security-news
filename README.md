...
ログイン/会員登録画面(`login-app/`, React + Vite)は**Vercelに単体デプロイ**し(https://security-news-beta.vercel.app/)、ダッシュボード本体は引き続きGitHub Pagesで公開する、2つのドメインをまたぐ構成です。

- 未ログインで GitHub Pages側の `index.html` / `blog.html` を開くと、自動的にVercel側のログイン画面へリダイレクトされます
- ログイン画面から会員登録(サインアップ)・ログインができ、ログイン成功後はGitHub Pages側のニュースページへ遷移します
...
- ログイン/会員登録画面は `login-app/`(React + Vite)。ビルド出力(`login-app/dist/`)はコミットせず、Vercel側で都度ビルドする
...
- `login-app/` — ログイン/会員登録画面のReact + Viteソース(Vercelに単体デプロイ、`vercel.json`でSPAのルーティング設定)
