...
- 手動「更新」ボタンとGeminiチャット機能も、Vercel(`login-app/api/`)のサーバーレス関数を経由して利用できます(下記「Vercel API機能」参照)
...
## Vercel API機能(手動更新・Geminiチャット)

GitHub Pages公開版の「更新」ボタンとGeminiチャットは、`login-app/api/` にあるVercelサーバーレス関数を経由して動作します。

- `api/ask.js` — Gemini APIへの質問応答を代理する(`GEMINI_API_KEY`をサーバー側に隠したまま利用)
- `api/trigger-refresh.js` — GitHub Actionsの収集ワークフロー(`update-pages.yml`)を即座に起動する。実際の収集・翻訳はVercelの実行時間に収まらないため、ここでは起動のみを行い、収集自体は数分後にGitHub Actions側で完了する

どちらもCORSで `https://arata5991.github.io` からのアクセスのみを許可しています。

### 設定手順

Vercelプロジェクトの `Settings` → `Environment Variables` で以下を追加してください。

1. `GEMINI_API_KEY` — https://aistudio.google.com/apikey で取得した無料枠のAPIキー(ローカル版と同じもので可)
2. `GH_WORKFLOW_TOKEN` — GitHubの [Fine-grained personal access token](https://github.com/settings/personal-access-tokens) を発行し、対象リポジトリを `security-news` のみに限定、Permissionsは `Actions: Read and write` のみを付与したもの。他の権限は不要です

設定後、Vercelで再デプロイ(Redeploy)してください。どちらか一方でも未設定の場合、該当する機能(チャット/更新ボタン)を呼び出した際にエラーメッセージが表示されるだけで、他の機能には影響しません。
...
- `login-app/api/ask.js` / `login-app/api/trigger-refresh.js` — 公開版から呼び出すVercelサーバーレス関数(Geminiチャット・手動更新のプロキシ)
